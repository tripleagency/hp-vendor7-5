...
/// Reusable area dropdown that respects current locale.
class AreaDropdownWidget extends StatelessWidget {
  final List<AreaEntity> areas;
  final AreaEntity? selectedArea;
  final bool isLoading;
  final void Function(AreaEntity?) onChanged;
  final String? Function(AreaEntity?)? validator;
  ...
  Widget build(BuildContext context) {
    ...
    return DropdownButtonFormField<AreaEntity>(
      ...
      decoration: InputDecoration(
        hintText: isLoading ? 'loading'.tr() : 'select_area'.tr(),
      ...
      items: areas.map((area) {
        final name = isRtl ? area.nameAr : area.nameEn;
